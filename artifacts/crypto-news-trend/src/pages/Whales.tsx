    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-lg">Real-time large transaction monitor across chains.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {filteredWhales.map((tx, i) => (
          ...
          >
            <div className={`px-4 py-2 text-[10px] font-bold tracking-wider flex justify-between items-center ${getCardColor(tx.type).split(' ').slice(1).join(' ')}`}>
              <span>{tx.type}</span>
            ...
            <div className="p-4">
              ...
              <div className="flex items-center justify-between text-xs font-mono bg-secondary/50 p-2 rounded-md">
                <div className="flex flex-col">
                  <span className="text-muted-foreground text-[9px] uppercase">From</span>
                  <span className="text-foreground">{tx.fromWallet}</span>
                </div>
                <ArrowRight className="w-4 h-4 text-muted-foreground shrink-0 mx-2" />
                <div className="flex flex-col text-right">
                  <span className="text-muted-foreground text-[9px] uppercase">To</span>
